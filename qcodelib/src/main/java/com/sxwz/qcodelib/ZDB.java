...
import com.sxwz.qcodelib.database.CursorHelper;
import com.sxwz.qcodelib.database.DaoConfig;
import com.sxwz.qcodelib.database.DbModel;
import com.sxwz.qcodelib.database.OneToManyLazyLoader;
import com.sxwz.qcodelib.database.SqlBuilder;
import com.sxwz.qcodelib.database.SqlInfo;
import com.sxwz.qcodelib.database.utils.KeyValue;
import com.sxwz.qcodelib.database.utils.ManyToOne;
import com.sxwz.qcodelib.database.utils.OneToMany;
import com.sxwz.qcodelib.database.utils.TableInfo;
...
public class ZDB {
    ...
    /** 用于保存所有数据库 */
    private static HashMap<String, ZDB> daoMap = new HashMap<String, ZDB>();

    private SQLiteDatabase db;
    private final DaoConfig config;

    private ZDB(DaoConfig config) {
        if (config == null) {
    ...
    private synchronized static ZDB getInstance(DaoConfig daoConfig) {
        ZDB dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new ZDB(daoConfig);
...
//     */
//    public static ZDB create(boolean isDebug) {
//        Context cxt = ZActivityStack.create().topActivity();
//        if (cxt == null) {
...
//    }
     ...
     * @param context
     */
    public static ZDB create(Context context) {
     ...
     *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
     */
    public static ZDB create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     *            数据库名称
     */
    public static ZDB create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static ZDB create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     *            数据库名称
     */
    public static ZDB create(Context context, String targetDirectory,
                             String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static ZDB create(DaoConfig daoConfig) {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    ZLog.d(getClass().getName() + e.getMessage());
                }
            }
        }
        if (cursor != null) {
    ...
    private void exeSqlInfo(SqlInfo sqlInfo) {
        if (sqlInfo != null) {
            ...
            ZLog.d(getClass().getName() + "sava error:sqlInfo is null");
        }
    }
